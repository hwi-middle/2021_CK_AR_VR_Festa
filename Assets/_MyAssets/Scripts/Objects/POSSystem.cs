...
public class POSSystem : MonoBehaviour
{
    ...
    private void Refresh()
    {
        for (int i = 0; i < _goodsList.Count; i++)
        {
            ...
            posRows[i].transform.GetChild(1).GetComponent<Text>().text = _goodsList[i].unitPrice.ToString();
            posRows[i].transform.GetChild(2).GetComponent<Text>().text = _goodsCount[i].ToString();
            posRows[i].transform.GetChild(3).GetComponent<Text>().text =
                (_goodsList[i].unitPrice * _goodsCount[i]).ToString();
        }
    }
}